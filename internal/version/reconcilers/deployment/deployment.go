...
func (r *Reconciler) Reconcile(ctx context.Context, version *corev1alpha1.Version) error {
	deploymentName := names.DeploymentName(version)
	...
	if apierrs.IsNotFound(err) {
		...
		_, err = r.createDeployment(ctx, deploymentName, version)
		if err != nil {
		...
		// deployment exists
		_, err = r.updateDeployment(ctx, deploymentName, version, deployment)
		if err != nil {
			return fmt.Errorf("failed to update deplyoment %q: %w", deploymentName, err)
		}

		// TODO: Surface deployment status to version
	}
...
func (r *Reconciler) createDeployment(ctx context.Context, name types.NamespacedName, version *corev1alpha1.Version) (*appsv1.Deployment, error) {
	deployment, err := makeDeployment(name, version)
	if err != nil {
...
func (r *Reconciler) updateDeployment(ctx context.Context, name types.NamespacedName, version *corev1alpha1.Version, in *appsv1.Deployment) (*appsv1.Deployment, error) {
	deployment, err := makeDeployment(name, version)
	if err != nil {
...
func makeDeployment(name types.NamespacedName, version *corev1alpha1.Version) (*appsv1.Deployment, error) {
