...
func (c *Client) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	is := &aiv1alpha1.InferencePipeline{}
	...
	if err != nil {
		...
		return ctrl.Result{}, fmt.Errorf("failed to retrieve latest inferencepipeline %s: %w", req.NamespacedName, err)
	}

	// reconcile resources
	for _, rec := range []func(context.Context, *aiv1alpha1.InferencePipeline) error{} {
